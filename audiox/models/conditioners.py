...
class CLIPWithSyncWithEmptyFeatureConditioner(Conditioner):
    ...
    def __init__(
        ...
        if self.clip_model_name=='clip-vit-base-patch32':
            ...
            self.Temp_pos_embedding = nn.Parameter(torch.randn(1, duration*video_fps, temporal_dim)) # spatial transformer:[1, 50*t*fps, 768]
    ...
    def forward(self, Video_list: tp.List[tp.Union[torch.Tensor, tp.Dict]], device: tp.Union[torch.device, str]) -> tp.Tuple[torch.Tensor, torch.Tensor]:
...
class AudioAutoencoderConditionerv2(Conditioner):
    ...
    def mask_audio(self, audio: torch.Tensor) -> torch.Tensor:
        ...
        Randomly mask audio
        Args:
            audio: tensor with shape [b, channels, length]
        Returns:
            masked_audio: masked audio with masked regions set to 0
        """
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any]) -> MultiConditioner:
    ...
    for conditioner_info in config["configs"]:
        ...
        elif conditioner_type == "audio_autoencoder_v2":
            sample_rate = conditioner_config.pop("sample_rate", None)
            assert sample_rate is not None, "Sample rate must be specified for pretransform conditioners"
            pretransform = create_pretransform_from_config(conditioner_config.pop("pretransform_config"), sample_rate=sample_rate)
            if conditioner_config.get("pretransform_ckpt_path", None) is not None:
